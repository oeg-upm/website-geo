...
class Config(object):
    ...
    def __init__(self):
        ...
        # MAIL CONFIGURATION
        self.mailer_port = settings['mail']['port']
        self.mailer_server = settings['mail']['server']
        self.mailer_host = settings['mail']['host'] if \
            'host' in settings['mail'] else self.mailer_server
        self.mailer_inbox = settings['mail']['inbox']
        self.mailer_sender = settings['mail']['sender']
        self.mailer_pwd = settings['mail']['password']

        # LOGGING
        formatter = logging.Formatter(
            "[%(asctime)s] %(levelname)s - %(message)s"
        )
        self.flask_logger = logging.Logger("portal-mailer-logger")
        ...
        self.error_logger = logging.Logger('portal-mailer-error-logger')
